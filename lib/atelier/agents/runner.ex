defmodule Atelier.Agents.Runner do
  ...
  def handle_info({:file_validated, filename}, state) do
    ...
    case Path.extname(filename) do
      ".js" -> execute_command("node", [full_path], filename, state)
      ".ex" -> execute_command("elixir", [full_path], filename, state)
      ".py" -> execute_command("python3", [full_path], filename, state)
  ...
  defp execute_command(cmd, args, filename, state) do
    ...
    case System.cmd(cmd, args, env: [], stderr_to_stdout: true) do
      {output, 0} ->
